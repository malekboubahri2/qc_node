...
typedef struct MqttCommand
{
    ...
    union
    {
        ...
        struct
        {
             ...
             *
             * NOTE: Only the topic is fully copied here.  Payload for QoS 0
             * is also copied.  For QoS 1/2 the payload pointer must remain
             * valid until PUBACK/PUBCOMP (coreMQTT retransmit requirement).
             * QoS 1/2 callers should use static or long-lived heap buffers.
             */
...
static void             mqtt_event_callback ( MQTTContext_t            *pCtx,
...
static MQTTStatus_t do_connect( void )
{
        ...
        && ( s_cfg.pConnectProps->will.pTopic != NULL ) )
    {
        ...
        if( cp->will.willDelayIntervalSec > 0U )
        {
            ...
            willProps.size    = sizeof( willPropBuf );
            ...
            case CMD_STOP:
                 ...
                 * reason code (0x00).  The broker will not send the Will. */
                {
                    ...
                    MQTTPropBuilder_t discProps = { .pBuffer = discPropBuf,
                                                    .size    = sizeof( discPropBuf ) };
                    MQTT_Disconnect( &s_mqttCtx, &discProps );
                }
                                  ...
                                  MQTTDeserializedInfo_t *pInfo )
{
    ...
    if( ( pPacket->type & 0xF0U ) == MQTT_PACKET_TYPE_PUBLISH )
    {
        if( ( pInfo != NULL ) && ( pInfo->pPublishInfo != NULL ) )
        {
             ...
             * for CONNACK, and is passed through pInfo for PUBLISH in v5. */
            dispatch_inbound( pInfo->pPublishInfo,
                              ( const MQTTPropBuilder_t * )pInfo->pPropBuilder );
        }
    }
                                      ...
                                      uint32_t                 timeoutMs )
{
    ...
    /* Copy payload inline for QoS 0 (if it fits); otherwise use the pointer. */
