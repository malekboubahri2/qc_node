...
{
    char     buf[ ESP01_AT_CMD_MAX_LEN ];
    uint16_t len;
    HAL_StatusTypeDef status;

    log_at_command( pCmd );

    len = ( uint16_t )snprintf( buf, sizeof( buf ), "%s\r\n", pCmd );
                                            ...
                                            uint32_t    timeoutMs )
{
    ...
    while( HAL_GetTick() < deadline )
    {
        ...
        {
            ESP01_LOG_ERROR( "WiFi scan returned ERROR\n" );
            return ESP01_ERR_AT;
        }
...
static ESP01_Status_t at_wait_wifi_query( bool *pConnected, uint32_t timeoutMs )
{
    ...
    while( HAL_GetTick() < deadline )
    {
        ...
        if( strcmp( line, "ERROR" ) == 0 )
        {
            ESP01_LOG_ERROR( "WiFi association query returned ERROR\n" );
            return ESP01_ERR_AT;
        }
...
ESP01_Status_t ESP01_WifiConnect( const char *pSsid, const char *pPassword )
{
    ...
    ESP01_Status_t ret = at_send_cmd( pActiveCtx->pUart, cmd );
...
ESP01_Status_t ESP01_IsWifiConnected( bool *pConnected )
{
    ...
    ret = at_send_cmd( pActiveCtx->pUart, "AT+CWJAP?" );
    if( ret != ESP01_SUCCESS )
    {
        return ret;
    }
                                            ...
                                            bool       *pTargetSeen )
{
    ...
    ret = at_send_cmd( pActiveCtx->pUart, "AT+CWLAP" );
    if( ret != ESP01_SUCCESS )
    {
        return ret;
    }
                               ...
                               uint16_t          port )
{
    ...
    ESP01_Status_t ret = at_send_cmd( pCtx->pUart, cmd );
    if( ret != ESP01_SUCCESS ) return ret;
                                   ...
                                   NetworkContext_t     *pCtx )
{
    ...
    pTransport->writev         = NULL;   /* not required by coreMQTT */
