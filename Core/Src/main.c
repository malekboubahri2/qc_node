...
int main(void)
{
  ...
  /* Start non-blocking reception for the debug UART only.
   * USART2 is owned by the ESP01 transport and is armed in ESP01_Init(). */
	HAL_UART_Receive_IT(&huart1, &rx, 1);

  /* Load cached configs from Octo-SPI flash (if any) */
  size_t prod_len = 0;
  char *prod = config_loader_get_products(&prod_len);
  if (prod) {
    printf("main: loaded products config (%zu bytes)\n", prod_len);
    free(prod);
  } else {
    printf("main: no products config cached\n");
  }

  size_t ops_len = 0;
  char *ops = config_loader_get_operators(&ops_len);
  if (ops) {
    printf("main: loaded operators config (%zu bytes)\n", ops_len);
    free(ops);
  } else {
    printf("main: no operators config cached\n");
  }
  /* USER CODE END 2 */
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOH, VSYNC_FREQ_Pin|RENDER_TIME_Pin|MCU_ACTIVE_Pin|RST_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOA, CH_PD_Pin|LCD_BL_CTRL_Pin|LCD_ON_OFF_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : CH_PD_Pin LCD_ON_OFF_Pin */
  GPIO_InitStruct.Pin = CH_PD_Pin|LCD_ON_OFF_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_PULLUP;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
...
int _write(int file, char *ptr, int len) {
    ...
    status &= HAL_UART_Transmit(&huart1, (const uint8_t *)"\n\r", 2, 100);
